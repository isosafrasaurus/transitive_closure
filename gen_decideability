...
Lemma index_inv_index : forall a : A, index_inv (index a) = a.
Proof.
  intros a. destruct a. simpl. reflexivity.
Qed.

Lemma index_index_inv : forall n : nat, index (index_inv n) = n.
Proof.
  intros n. simpl. reflexivity.
Qed.

(* Example relation as an adjacency matrix *)
Definition example_relation (i j : nat) : bool :=
  match (i, j) with
  | (0, 1) => true
  | (1, 2) => true
  | _ => false
